...
class SearchResult extends React.Component {
	...
	getSearchResult = (data) => {
		...
		if(cat_filters) {
			var cat_array = [];
			cat_filters.forEach((item, index) => {
				if(item.key !== ""){
					var category_dict = {}
		...
		if(source_filters) {
			var source_array = [];
			source_filters.forEach((item, index) => {
				if(item.key !== ""){
					var source_dict = {}
		...
		if(hashtags_filters) {
			var hashtags_array = [];
			hashtags_filters.forEach((item, index) => {
				if(item.key !== ""){
					var hashtags_dict = {}
	...
	queryFilter = (data, checked) => {
		...
		if (window.history.pushState) {
			getRequest(ARTICLE_POSTS+"?"+DOMAIN+"&q="+this.state.query+"&"+final_query, this.getSearchResult);
			var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname +"?q="+this.state.query;
			if(final_query){
	...
	componentDidMount() {
		...
		if(this.state.final_query){
			...
			getRequest(ARTICLE_POSTS+"?"+DOMAIN+"&q="+this.state.query, this.getSearchResult);
		}
	}
										...
										<div className="clerfix">
											<div className="float-left">
												<div className="search">
													Search result: <span className="text-capitalize">{this.state.query}</span>
												</div>
											</div>
